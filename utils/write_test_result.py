...
# sudo pip3 install bioservices, biopython
...
from utils.helpers import get_stop_dic, pos_surround
from utils.helpers import makeEasyTag, Indent, postprocess, cos_sim, extract_id_from_res
...
def searchEntityId(s, predLabels, entity2id, text_byte, all_id):
    '''
    对一个句子中的所有实体进行ID链接：
...
def entity_disambiguation_cnn(entity, entity_id, cnn, x_sen, x_data,
                              pos_sen, x_id_dict, position, stop_word, leixing, prob, word_index, id2def, dict_or_text):
    type_id = ''
    ...
    fea_list = {'x_left': [], 'x_right': [], 'x_left_pos': [], 'x_right_pos': [],
                'x_id': [], 'x_id_index': [], 'x_elmo_l': [], 'x_elmo_r': []}
    ...
    for i in range(len(entity_id)):
        ...
        if dict_or_text=='id':
            if id in x_id_dict:
                ...
                continue
            x_id = [x_id]
        elif dict_or_text=='def':
            if id in x_id_dict:
                fea_list['x_id_index'].append([x_id_dict[id]])
            else:
                entity_id_new.pop()
                continue

            # 将ID转换为DEF,DEF再序列化
            id_def = id2def.get(id)
            if id_def:
                id_def_index = []
                # 对标点符号进行切分，但保留 ^ 用作标记识别符
                for special in "!\"#$%'()*+,-./:;<=>?@[\\]_`{|}~":
                    id_def = id_def.replace(special, ' '+special+' ')
                id_def = id_def.replace('   ', ' ').replace('  ', ' ')
                # 对ＩＤ描述文本进行序列化
                for one in id_def.split():
                    id_def_index.append(word_index[one] if one in word_index else 0)
                # print(x_id, id_def_index)
                x_id = id_def_index
            else:
                x_id = [0]
                # entity_id_new.pop()
    ...
    if not fea_list['x_id']:
        # print('空空如也') # ①id集合只有[protein:,gene:] ② 集合中的id未包含在x_id_dict
        if leixing == 'protein':
            type_id = 'protein:' + entity
        else:
            type_id = 'gene:' + entity
    ...
    while len(fea_list['x_id']) < batch_size:
        ...
        # fea_list['x_id_index'].append(fea_list['x_id_index'][0])
    ...
    # dataSet = [fea_list['x_id'], fea_list['x_left'], fea_list['x_right']] # old
    # dataSet = [fea_list['x_id_index'], fea_list['x_id'], fea_list['x_left'], fea_list['x_right']]
...
def writeOutputToFile(path, predLabels, ned_model, prob, word_index, id2def, dict_or_text='id'):
    '''
    按顺序读取原文件夹中的xml格式文件
    ...
    if ned_model:
        print('new!')
        with open('ned/data/x_id_dict.pkl', 'rb') as f:
    ...
    idx2pos = {}
    with open('data/pos2idx.txt') as f:
        for line in f:
            pos, idx = line.split('\t')
            idx2pos[idx.strip('\n')] = pos
        ...
        if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
            ...
            for doc in documents:
                ...
                for passage in passages:
                    ...
                    if "SyGCaMP5 and MtDsRed" in text:
                        print("SyGCaMP5 and MtDsRed")
                        print(f)
                    ...
                    for annotation in annotations:
                        ...
                        if ID.startswith('gene') or ID.startswith('protein') or ID.startswith(
                            ...
                            entity2golden[a] = ID   # 统计当前passage中的 [golden实体：ID］
                    ...
                    for i in range(len(entity_list)):
                        ...
                        if goldenID and goldenID not in visited: # 未访问过
                            visited.append(goldenID)
                            if goldenID in entity_id:
                                num_exit += 1
                                if goldenID == type_id:
                                    num_match += 1
                                else:
                                    num_incorrect_ID += 1  # id分配错误　incorrect ID
                            else:
                                num_missed_ID += 1  # id分配错误　missed ID
                    ...
                    num_pner_fn += (len(entity2golden)-len(entity2golden_empty)) # 实体丢失 PNER FN
    ...
    print('\n测试集预测结果写入成功！')
    print('统计结果如下：')
    print('PNER的错误分析-以脚本测得的corpus_scores.csv为准(其中,hyponly→PNER FP,refonly→PNER FN)')
    print('num_missed_ID:　{}'.format(num_missed_ID))  # 1376 PNEN
    print('num_incorrect_ID:　{}'.format(num_incorrect_ID))  # 725 PNEN
    print('{}个词未找到对应的ID'.format(num_entity_no_id))  # 354
    print('{}个词有歧义'.format(len(words_with_multiId)))  # 2065
    print('完结撒花\n')

    # # 将实体ID集合提前保存
    # with open('all_id.txt', "w") as f:
    #     for line in all_id:
    #         f.write('\t'.join(line))     
    #         f.write('\n')
