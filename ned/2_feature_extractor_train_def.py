...
def searchEntityId(entity, entity2id):
    ...
    if v1 in gene2id:
        ...
        Ids = search_id_from_NCBI(query_list) # NCBI-gene数据库API查询
        if Ids:
            id_list.extend(Ids)
            # descrip_list.extend(descriptions)
            entity2id[v1] = Ids if v1 not in entity2id else list(set(entity2id[v1] + Ids))
...
def get_x_y(entity, id, x_sen, x_data, pos_sen, position, res):
    ...
    '''
    x_left, x_pos_left, x_right, x_pos_right, x_id, x_id2, y, x_elmo_l, x_elmo_r = res
    ...
    for i in range(len(x_id_one)):
        ...
        if id_def:
            ...
            x_id_def.append(id_def_index)
            x_id_one_new.append(id_one)
        else:
            x_id_def.append([0])
            x_id_one_new.append(id_one)
            # continue
    ...
    # 消歧模型的训练数据组装
    for i in range(len(x_id_one_new)):
        # des_one = x_des_one[i]
        id_def = x_id_def[i]
        id_one = x_id_one_new[i]
        label = labels[i]
...
if __name__ == '__main__':
    ...
    id2def = {}
    with open('data/definition/definition.txt') as f:
        for line in f:
            splited = line.split('\t')
            eid, definition = splited[0], ' '.join(splited[1:])
            id2def[eid] = definition
    with open('data/definition/gene_def.txt') as f:
        for line in f:
            splited = line.split('\t')
            eid, definition = splited[0], ' '.join(splited[1:])
            id2def[eid] = definition
    ...
    # x_des, x_des_test = [], []
    y, y_test = [], []
    res_list = [x_left, x_pos_left, x_right, x_pos_right, x_id, x_id2, y, x_elmo_l, x_elmo_r]
    res_test_list = [x_left_test, x_pos_left_test, x_right_test, x_pos_right_test, x_id_test, x_id_test2, y_test, x_elmo_l_t, x_elmo_r_t]
    ...
    # 对样例的ID进行序列化
    x_id_dict = {value:idx+1 for idx, value in enumerate(list(set(x_id2 + x_id_test2)))}  # 只包含golden实体
    ...
    for id in x_id2:
        index = x_id_dict[id]
        x_id_index.append([index])

    # x_id_new_test = []
    # for id2 in x_id_test2:
    ...
    # 保存ID Embedding
    with open('data/data_train_id_def.pkl', "wb") as f:
        res = (x_id, x_id_index, id2def, id_embedding)
        pkl.dump(res, f, -1)
